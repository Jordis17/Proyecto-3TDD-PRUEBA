...
//
// Adaptado del testbench del Proyecto 2 al nuevo orden de registros
// (CONTROL = 00, TX = 01, RX = 10). El periferico se conecta al nucleo
// real (uart_core con UART_tx.vhd y UART_rx.vhd) y la linea de
...
// linea, deserializacion, registro RX.
//
// Para simular rapido se usan divisores cortos: 32 ciclos por bit en
// transmision y 2 x 16 = 32 en recepcion. Con los valores reales
// (434 y 27) solo cambia la duracion.
//
// Comprueba:
...
module tb_uart_peripheral;
    ...
    localparam int BAUD_DIV     = 32;   // corto, para simular rapido
    localparam int BAUD_X16_DIV = 2;
    localparam int CICLOS_TRAMA = BAUD_DIV * 10;

    localparam logic [1:0] ADDR_CTRL = 2'b00;
    localparam logic [1:0] ADDR_TX   = 2'b01;
    localparam logic [1:0] ADDR_RX   = 2'b10;
    ...
    uart_core #(.BAUD_DIV(BAUD_DIV), .BAUD_X16_DIV(BAUD_X16_DIV)) nucleo (
        .clk_i(clk), .rst_i(rst),
    ...
    initial begin
        ...
        $display("");
        if (errores == 0)
            $display("RESULTADO: PASS (registros, semantica por bit, independencia de campos y trama completa)");
        else
            $display("RESULTADO: FAIL (%0d comprobaciones fallidas)", errores);
        $display("");
        $finish;
    end
